...
testF(TestAbsenceOfNAfterQWithinThreeSecondsProperty, Property_switches_from_satisfied_to_violated_and_back) {
  //given
  AbsenceOfNAfterQWithinThreeSecondsProperty automaton;

  //when
  std::vector<uint8_t> events = {EVENT_Q, EVENT_S};
  ...
  //when 
  std::vector<uint8_t> events2 = {EVENT_Q, EVENT_N};
  ...
  //when
  std::vector<uint8_t> events3 = {EVENT_Q, EVENT_S};
  std::vector<uint32_t> timestamps3 = {7000, 11000};
