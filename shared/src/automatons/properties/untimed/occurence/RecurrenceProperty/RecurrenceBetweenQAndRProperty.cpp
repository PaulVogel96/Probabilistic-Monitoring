...
class RecurrenceProperty : public ProbStatemachine {
  public:
    RecurrenceProperty() : ProbStatemachine() {
      ...
      static ProbTransition t2(&q_happened, &q_happened, 0.01, 'P');
      static ProbTransition t3(&q_happened, &p_happened, 0.99, 'P');
      static ProbTransition t4(&p_happened, &not_p_happened, 1.0, 'X');
      static ProbTransition t5(&not_p_happened, &r_happened, 1.0, 'R');
      static ProbTransition t6(&q_happened, &r_happened, 1.0, 'R');
      static ProbTransition t7(&p_happened, &r_happened, 1.0, 'R');
      static ProbTransition t8(&r_happened, &q_happened, 1.0, 'R');
